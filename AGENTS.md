...
|File|Purpose|
|---|---|
|`src/index.js`|`createNode`: Corestore, Autobee tracker + `apply`, Hyperblobs, blob server, Hyperswarm, `search` / `publish` / `remove`|
|`src/acquire.js`|Private job queue: fetch a source, `publish` it. Stored in `jobs.json`, never in the Corestore|
|`src/http.js`|Bearer-secret `/v1` machine API|
|`bin/relay.js`|Relay process, configured by env|
|`test/network.test.js`|Two relays on a local HyperDHT testnet|
