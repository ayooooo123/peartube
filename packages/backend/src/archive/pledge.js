...
export function createArchivePledge(input = {}) {
  ...
  const envelope = createApplicationEnvelope({ recordType: ARCHIVE_PLEDGE_RECORD_TYPE, body: encodeCanonical(body), keyPair: input.keyPair, issuedAt: input.issuedAt, expiresAt: input.retentionUntil })
...
export async function verifyArchivePledge(envelope, options = {}) {
  ...
  const verified = await verifyApplicationEnvelope(envelope, { recordType: ARCHIVE_PLEDGE_RECORD_TYPE, now: options.now, allowedSigners: [allowedSigner] })
