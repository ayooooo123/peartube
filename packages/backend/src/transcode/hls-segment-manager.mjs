...
export class HlsSegmentManager {
  ...
  generatePlaylist(options = {}) {
    ...
    logDebug('[HlsSegmentManager] generatePlaylist: total segments in map:', this.segments.size, 
    ...
    if (playlistSegments.length === 0) {
      ...
      logDebug('[HlsSegmentManager] generatePlaylist: returning empty playlist (no segments yet)')
      return [
        '#EXTM3U',
        '#EXT-X-VERSION:3',
    ...
    for (const segment of playlistSegments) {
      ...
      if (hostForPlaylist) {
        lines.push(`http://${hostForPlaylist}/hls/${sessionId}/segment${segment.index}.ts`)
      } else {
    ...
    logDebug('[HlsSegmentManager] Playlist generated with', playlistSegments.length, 'segments, last index:', lastSegmentIndex, 'isComplete:', this.isComplete)
    ...
    logDebug('[HlsSegmentManager] Playlist content before preview:', playlist)
    const preview = playlist.split('\n').slice(0, 8).join('\n')
    logDebug('[HlsSegmentManager] generatePlaylist preview:\n' + preview + '\n...')
