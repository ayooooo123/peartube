...
test('configures local PearTube blob streams without an OkHttp read timeout', () => {
  ...
  assert.match(source, /withPearTubeBlobRangeHeader\(dataSpec\)/)
  assert.match(source, /withAdditionalHeaders\(mapOf\("Range" to rangeValue\)\)/)
  assert.match(source, /PearTube: local blob-server applying Range/)
