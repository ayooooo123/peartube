...
| Native macOS | Experimental SwiftUI shell | `npm run desktop:native:build` | tests and release archive in `build-desktop.yml` |
...
- Protocol version is centralized in `packages/host/src/contracts.js` and currently validated by the protocol client and native macOS bridge.
...
- Schema generation now produces JS HRPC/schema output plus Swift schema/HRPC support for native desktop via `npm run schema:full`.
- Electrobun desktop is off the old `pear run` path and uses embedded `pear-runtime`.
- Native macOS has a SwiftUI shell, generated HRPC types, sidecar/worklet bundle generation, and Xcode test/build coverage.
...
- Generated JS schema output under `packages/spec/spec/` is tracked. Native desktop Swift support and resource bundles are regenerated by documented commands.

## Known Constraints

- Pear OTA desktop release flow is not wired. Use the Electrobun/native desktop release artifact workflows, and see `docs/pear-runtime-evolution-readiness.md` before adding OTA automation.
- Native macOS is experimental and should be treated as a client shell over the universal backend, not a separate backend.
