...
const entryFile = path.join(packageRoot, 'Bridge', 'barekit-corestore-worklet.cjs')
const bundleFile = path.join(packageRoot, 'Resources', 'Generated', 'barekit-corestore.bundle')
...
if (forced || missingBundle || staleBundle) {
  ...
  console.log(`[bundle:barekit-corestore:ensure] Rebuilding (${reason})`)
  runBarePack()
} else {
  console.log('[bundle:barekit-corestore:ensure] BareKit corestore bundle is up to date')
}
