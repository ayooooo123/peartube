...
/** Failures that a different equivalent source might not share. */
const RETRYABLE_ERROR_CODES = new Set(['PEER_TIMEOUT', 'PEER_DISCONNECT', 'SESSION_LIMIT', 'RANGE_MISMATCH'])

/**
 * Failures that describe the title or this device, not the attempted source.
 * Retrying another source cannot change them, so preparation stops immediately.
 */
...
export async function preparePlaybackSource(options = {}) {
  ...
  for (const source of order) {
    ...
    try {
      ...
      attempts.push({ publicationId, errorCode: lastErrorCode })
      if (TERMINAL_ERROR_CODES.has(lastErrorCode)) break
      continue
    } finally {
