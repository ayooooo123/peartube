...
test('archive UI commands and flags are exposed by the relay CLI', async (t) => {
  ...
  const packageCompose = readFileSync(join(__dirname, '..', 'docker-compose.example.yml'), 'utf8')
  ...
  t.ok(compose.includes('peartube-public-relay:'), 'root relay compose includes a public relay example')
  t.ok(compose.includes('peartube-curated-relay:'), 'root relay compose includes a curated relay example')
  t.ok(compose.includes('peartube-archive-publisher:'), 'root relay compose includes an archive publisher example')
  t.ok(compose.includes('PEARTUBE_NODE_ROLES: public-index,relay-cache'), 'root relay compose declares public relay/cache roles')
  t.ok(compose.includes('PEARTUBE_POLICY: allowlist'), 'curated relay example uses allowlist policy')
  t.ok(compose.includes('PEARTUBE_MODERATION_MODE: enforce'), 'curated relay example enforces moderation rules')
  t.ok(compose.includes('PEARTUBE_NODE_ROLES: archiver,public-index,relay-cache'), 'archive publisher example declares publisher role')
  t.ok(compose.includes('PEARTUBE_ARCHIVE_ENABLED: "true"'), 'archive publisher example enables polling archive mode')
  ...
  t.ok(packageCompose.includes('peartube-public-relay:'), 'packaged compose example includes a public relay')
  t.ok(packageCompose.includes('peartube-curated-relay:'), 'packaged compose example includes a curated relay')
  t.ok(packageCompose.includes('peartube-archive-publisher:'), 'packaged compose example includes an archive publisher')
})
