...
cold-start `BootstrapIO` must verify a live cookie-bound direct active challenge
for the exact guard advertisement and observed endpoint. After readiness,
...
no unauthenticated branch feature that disables storage. Before READY, the exit
may perform only bounded DHT/storage discovery, active validation, and caching
of candidate evidence. It has no branch handle secret or destination table and
must not mint a `DESTINATION_REF_V1`, construct a semantic seed object, sign
`DHT_EXIT_SEEDS_V1`, or cache its bytes.

After validating ACK and entering OPEN, Section 10.1 creates the fresh 32-byte
branch handle secret and empty table. The exit rechecks every candidate's
provenance, identity/ID, endpoint, capability, advertisement digest/epoch, and
expiry, then mints the references. Only after at least one valid DHT reference
...
one DHT reference or one storage pair, zero count, candidate recheck failure,
handle mint/table insertion failure, object construction/signature failure,
conflict, or lateness destroys the branch and erases the handle secret, table,
candidate evidence, and partial seed state. No partial seed set is sent or
cached, and failure never triggers direct bootstrap, legacy fallback, or
caller-selected probing.
...
Secret/table creation occurs exactly once, after a valid ACK causes the exit to
enter OPEN. Before OPEN, validated candidate evidence grants no handle
authority: the exit has no branch handle secret/table and cannot mint or cache
a destination reference. If post-OPEN seed minting or signing fails under
Section 6.5, branch destruction erases the new secret, every table entry,
candidate evidence, and all partial or cached seed state.
...
| `0x0101/v1` | `ROUTED_REQUEST_V1`                   | forward `ROUTE_PAYLOAD`                      | context AEAD + immutable exact nine-entry M3-v1 exit-origin policy bound during finalization | 1,382 / 1,390             | Native DHT-RPC Transport / Command Policy      | private-routes       |
...
| Initial exit seed delivery            | Every branch prevalidates bounded candidate evidence before READY, then after OPEN creates its handle secret/table and atomically mints/signs `DHT_EXIT_SEEDS_V1` with 1..3 DHT refs and 1..5 storage pairs; no partial set; five-second private-readiness deadline | Prevents pre-OPEN handle authority and unauthenticated feature negotiation. Post-OPEN mint/sign can fail after handshake and force branch teardown; pre-minting would improve apparent readiness but would create handles without the generation secret/state that authorizes them.                    |
...
- **Candidate evidence is not seed authority:** before OPEN the exit may cache
  only bounded validated candidate evidence. A valid ACK creates the branch
  handle secret/table; only then may the exit recheck candidates, mint handles,
  and atomically build/sign/cache the complete seed object. Any failure before
  the required DHT/storage minima destroys the branch and erases secret, table,
  evidence, and partial seed state; no partial set is sent.
