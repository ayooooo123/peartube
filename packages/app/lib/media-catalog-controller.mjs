...
function initialLoadingPatch(state, append) {
  const hasItems = state.items.length > 0
  return {
    status: hasItems ? 'ready' : 'loading',
    refreshing: !append && hasItems,
    loadingMore: append,
    errorCode: undefined,
    error: undefined,
  }
...
function catalogErrorPatch(resultOrError) {
  return {
    status: 'error',
    errorCode: resultOrError?.errorCode || resultOrError?.code || 'MEDIA_CATALOG_ERROR',
    error: resultOrError?.error || resultOrError?.message || 'Unable to load the media catalog',
    refreshing: false,
    loadingMore: false,
  }
}

function catalogSuccessPatch(state, result, append) {
  const incoming = Array.isArray(result?.items) ? result.items : []
  const baseItems = append ? state.items : []
  const validCursor = typeof result?.nextCursor === 'string' && result.nextCursor.length > 0
  return {
    status: 'ready',
    items: mergeEntities(baseItems, incoming),
    nextCursor: validCursor ? result.nextCursor : undefined,
    errorCode: undefined,
    error: undefined,
    refreshing: false,
    loadingMore: false,
...
export function createMediaCatalogController({ getMediaCatalog, pageSize = DEFAULT_PAGE_SIZE }) {
  ...
  const requestPage = async ({ append }) => {
    ...
    if (append && (state.loadingMore || !state.nextCursor)) return state
    const cursor = append ? state.nextCursor : undefined
    const sequence = ++requestSequence

    publish(initialLoadingPatch(state, append))

    try {
      const result = await getMediaCatalog({ cursor, limit })
      if (destroyed || sequence !== requestSequence) return state
      if (!result?.success) {
        publish(catalogErrorPatch(result))
        return state
      }
      publish(catalogSuccessPatch(state, result, append))
    } catch (error) {
      if (destroyed || sequence !== requestSequence) return state
      publish(catalogErrorPatch(error))
    }
    return state
  }
