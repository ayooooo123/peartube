...
const encoding158 = {
  preencode(state, m) {
    ...
    if (version >= 3 && m.bytesDownloaded) c.uint.preencode(state, m.bytesDownloaded)
    if (version >= 3 && m.totalBytes) c.uint.preencode(state, m.totalBytes)
  },
  encode(state, m) {
    const flags =
      ((version >= 3 && m.bytesDownloaded) ? 1 : 0) |
      ((version >= 3 && m.totalBytes) ? 2 : 0)

    c.string.encode(state, m.id)
    c.uint.encode(state, m.progress)
    c.uint.encode(state, flags)

    if (version >= 3 && m.bytesDownloaded) c.uint.encode(state, m.bytesDownloaded)
    if (version >= 3 && m.totalBytes) c.uint.encode(state, m.totalBytes)
  },
  decode(state) {
    const r0 = c.string.decode(state)
    const r1 = c.uint.decode(state)
    ...
    return {
      id: r0,
      progress: r1,
      bytesDownloaded: (version >= 3 && (flags & 1) !== 0) ? c.uint.decode(state) : 0,
      totalBytes: (version >= 3 && (flags & 2) !== 0) ? c.uint.decode(state) : 0
    }
  }
}
