...
test('channel view preserves publicBeeKey across native and web navigation/data fetches', () => {
  ...
  assert.match(nativeVideo, /function canAttemptPlayback/, 'native watch route should validate fetched metadata before playback')
  assert.match(nativeVideo, /if \(!canAttemptPlayback\(videoData\)\) \{[\s\S]*?setLoadErrorText\('Video metadata unavailable'\)[\s\S]*?setIsLoading\(false\)[\s\S]*?return/s, 'native watch route should not call preparePlayback for Unknown metadata fallbacks')
  assert.doesNotMatch(nativeVideo, /console\.warn\('\[VideoPlayer\] Playback skipped/, 'metadata fallback guard should not trigger the in-app dev warning banner')
