 ...
 *
 * Attaches HRPC handler implementations to the backend object.
 * These are looked up at call-time by registerSharedHandlers via
 * getSharedHandler(backend, name) -> backend[camelCase(name)].
 *
 * Same pattern as the desktop worker (pear-src/workers/core/index.ts).
 */
...
export function attachMobileHandlers(B, deps) {
  ...
  B.getPublicFeed = async () => { const r = await api.getPublicFeed(); return { entries: r.entries.map(e => ({ channelKey: e.driveKey || e.channelKey, publicBeeKey: e.publicBeeKey || null, channelName: e.name, videoCount: e.videoCount || 0, peerCount: e.peerCount || 0, lastSeen: e.lastSeen || 0 })) } }
