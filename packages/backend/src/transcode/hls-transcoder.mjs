...
const HLS_DEBUG = typeof process !== 'undefined' &&
  process?.env?.PEARTUBE_HLS_DEBUG === '1'

const logDebug = (...args) => {
  if (HLS_DEBUG) console.log(...args)
}

const hypercoreDirectMaxMbEnv = typeof process !== 'undefined'
  ? process?.env?.PEARTUBE_HYPERCORE_DIRECT_MAX_MB
  : undefined
const hypercoreDirectMaxBytes = (() => {
  const parsed = Number.parseInt(hypercoreDirectMaxMbEnv || '', 10)
...
function isSessionMatch(session, sourceKey, sourceUrl) {
  if (!session) return false
  if (sourceKey) return session.sourceKey === sourceKey
  return session.sourceUrl === sourceUrl
}

function markSessionCancelled(session, reason) {
  if (!session || session.cancelled) return
  session.cancelled = true
  session.cancelReason = reason || 'cancelled'
  if (session.status && session.status !== 'complete' && session.status !== 'error') {
...
function cleanupSession(session, options = {}) {
  if (!session) return
  const { destroySegments = false, remove = false } = options

  if (!session.inputCleaned) {
    ...
    if (session.hypercoreStreamReader) {
      try { session.hypercoreStreamReader.destroy() } catch {}
    }
    if (session.hypercoreReader) {
      try { session.hypercoreReader.destroy() } catch {}
    }
    session.inputCleaned = true
  }

  if (destroySegments && !session.segmentsDestroyed) {
    if (session.segmentManager) {
      try { session.segmentManager.destroy() } catch {}
    }
    session.segmentsDestroyed = true
  }
...
function getAvccLengthSize(avcc) {
  if (!avcc || avcc.length < 5 || avcc[0] !== 0x01) return null
  const lengthSizeMinusOne = avcc[4] & 0x03
...
function convertAvccPacketToAnnexB(packetData, lengthSize) {
  if (!packetData || lengthSize < 1 || lengthSize > 4) return null
  const parts = []
  let offset = 0
  ...
  while (offset + lengthSize <= packetData.length) {
    ...
    for (let i = 0; i < lengthSize; i++) {
      nalLen = (nalLen << 8) | packetData[offset + i]
    }
    offset += lengthSize

    if (nalLen <= 0 || offset + nalLen > packetData.length) break
    parts.push(Buffer.from([0, 0, 0, 1]))
    parts.push(packetData.slice(offset, offset + nalLen))
    offset += nalLen
  }
...
function scanH264NalFlags(packetData, maxScanBytes = 2000) {
  if (!packetData || packetData.length < 4) {
    return { hasSPS: false, hasPPS: false, hasIDR: false, lengthSize: null }
  }
  ...
  const limit = Math.min(packetData.length - 4, maxScanBytes)
  ...
  for (let i = 0; i < limit; i++) {
    ...
    if (packetData[i] === 0 && packetData[i + 1] === 0 && packetData[i + 2] === 1) {
      sawStartCode = true
  ...
  if (sawStartCode) {
    return { hasSPS, hasPPS, hasIDR, lengthSize: null }
  }

  // Fallback: length-prefixed NAL units (AVCC)
  ...
  for (const lengthSize of lengthSizes) {
    ...
    let lpHasPPS = false
    let lpHasIDR = false
    const maxOffset = Math.min(packetData.length, maxScanBytes)

    while (offset + lengthSize <= packetData.length && offset < maxOffset) {
      let nalLen = 0
      for (let i = 0; i < lengthSize; i++) {
        nalLen = (nalLen << 8) | packetData[offset + i]
      }
      ...
      if (nalLen <= 0 || offset + nalLen > packetData.length) {
        break
      }

      const nalType = packetData[offset] & 0x1f
      if (nalType === 7) lpHasSPS = true
      if (nalType === 8) lpHasPPS = true
      if (nalType === 5) lpHasIDR = true
    ...
    if (parsed > 0) {
      return { hasSPS: lpHasSPS, hasPPS: lpHasPPS, hasIDR: lpHasIDR, lengthSize }
    }
  }
...
/**
 * Parse HTTP range header for segment requests
 */
...
function retainResponseBuffer(res, buffer) {
  if (!res || !buffer) return
  res._ptResponseBuffer = buffer
  const clear = () => {
    if (res._ptResponseBuffer === buffer) res._ptResponseBuffer = null
  }
...
async function handleHttpRequest(req, res) {
  try {
    ...
    try {
      ...
      urlQuery = parsedUrl.search || ''
    } catch {}

    logDebug('[HlsTranscoder] HTTP request:', req.method, rawUrl)
    logDebug('[HlsTranscoder] Active sessions:', Array.from(sessions.keys()).join(', ') || 'none')
    ...
    const playlistMatch = urlPath.match(/^\/hls\/([^\/]+)\/stream\.m3u8/)
    const segmentMatch = urlPath.match(/^\/hls\/([^\/]+)\/segment(\d+)\.ts/)
    ...
    if (playlistMatch) {
      ...
      logDebug('[HlsTranscoder] 404 - URL pattern not matched:', rawUrl, 'path:', urlPath, urlQuery ? 'query:' + urlQuery : '')
      res.statusCode = 404
      res.end('Not found - invalid URL pattern')
      return
    }

    logDebug('[HlsTranscoder] Request for session:', sessionId, 'type:', requestType, 'segment:', segmentIndex)

    const session = sessions.get(sessionId)
    if (!session) {
      console.log('[HlsTranscoder] 404 - Session not found:', sessionId)
      logDebug('[HlsTranscoder] Known sessions:', Array.from(sessions.keys()))
      res.statusCode = 404
      res.end('Session not found: ' + sessionId)
      return
    }
    if (session.cancelled || session.status === 'cancelling' || session.status === 'cancelled') {
      console.log('[HlsTranscoder] 410 - Session cancelled:', sessionId)
      res.statusCode = 410
      res.end('Session cancelled')
      return
    }
    if (session.status === 'error') {
      console.log('[HlsTranscoder] 500 - Session error:', sessionId)
      res.statusCode = 500
      res.end('Session error: ' + (session.error || 'unknown'))
      return
    }
    ...
    if (requestType === 'playlist') {
      ...
      logDebug('[HlsTranscoder] Playlist preview:\n' + playlistPreview)
      if (playlist.split('\n').length > 20) {
      ...
      retainResponseBuffer(res, playlistBuf)
      res.end(playlistBuf)
      return
    }
    ...
    if (requestType === 'segment') {
      ...
      if (!segmentManager.hasSegment(segmentIndex)) {
        console.warn('[HlsTranscoder] 503 - Segment', segmentIndex, 'not ready (highest:', highestAvailable, ')')
        res.statusCode = 503
      ...
      if (!segmentData) {
        console.warn('[HlsTranscoder] 404 - Segment data null:', segmentIndex)
        res.statusCode = 404
      ...
      if (range) {
        ...
        const responseBuf = Buffer.from(segmentData.subarray(range.start, range.end + 1))
        logDebug('[HlsTranscoder] Serving segment', segmentIndex, 'range', range.start + '-' + range.end,
          '- size:', Math.round(sliceLen / 1024), 'KB')
        res.statusCode = 206
        res.setHeader('Content-Range', `bytes ${range.start}-${range.end}/${segLen}`)
        res.setHeader('Content-Length', sliceLen)
        retainResponseBuffer(res, responseBuf)
        res.end(responseBuf)
      } else {
        logDebug('[HlsTranscoder] Serving segment', segmentIndex, '- size:', Math.round(segLen / 1024), 'KB')
        res.statusCode = 200
        res.setHeader('Content-Length', segLen)
        retainResponseBuffer(res, segmentData)
        res.end(segmentData)
      }
    }
  } catch (err) {
 ...
 */
async function detectTranscodeNeeded(url, title = '') {
  return probeMedia(url, title)
}
...
async function hlsRemux(session, inputIO, segmentManager, totalSize, onProgress) {
  ...
  try {
    ...
    if (session.cancelled) {
      console.log('[HlsTranscoder] Remux cancelled, skipping finalization')
      return
    }
    ...
    if (outputIO && !outputOwnsIO) { try { outputIO.destroy() } catch {} outputIO = null }
...
async function hlsTranscodeVideo(session, inputIO, segmentManager, totalSize, onProgress) {
  ...
  let hwDevice = null
  let videoToolboxTransferFrame = null
  let videoToolboxTransferFormat = null
  let videoToolboxTransferFormatName = null
  let videoToolboxTransferFormatSource = null
  let videoToolboxValidFormats = null
  let videoToolboxTransferFailures = new Set()
  let videoToolboxGetFormatLogged = false
  let audioFrame = null
  let resampledFrame = null
  let outputPacket = null
  let convertedPacket = null
  let releaseAllPendingVideoFrames = null
  ...
  try {
    ...
    const targetWidth = videoStream.codecParameters.width
    const targetHeight = videoStream.codecParameters.height
    const videoBitDepth = Math.max(
      videoStream.codecParameters.bitsPerRawSample || 0,
      videoStream.codecParameters.bitsPerCodedSample || 0
    )
    ...
    const usesVideoToolbox = h264Selection.isHardware && h264Selection.name === 'h264_videotoolbox'
    const forceSoftwareDecode = session?.forceSoftwareDecode && videoToolboxDecodeSource !== 'env'
    if (forceSoftwareDecode && videoToolboxDecodeEnabled) {
      console.log('[HlsTranscoder] Forcing software decode for this session (VideoToolbox decode override)')
    }
    const usesVideoToolboxDecode = usesVideoToolbox && videoToolboxDecodeEnabled && !forceSoftwareDecode
    const usesVideoToolboxHwMap = usesVideoToolboxDecode && videoToolboxHwMapEnabled
    let videoToolboxUseHwMap = usesVideoToolboxHwMap
    let videoToolboxTransferErrorCount = 0
    // Hardware encoders can consume frames asynchronously; reuse can corrupt memory.
    const useFreshEncoderFrames = usesVideoToolbox
    if (usesVideoToolboxDecode) {
      const transferMethod = usesVideoToolboxHwMap ? 'hwMap' : 'transferData'
      console.log('[HlsTranscoder] VideoToolbox encoder detected - enabling hardware decode (' + transferMethod + ') + pooled frames')
    } else if (usesVideoToolbox) {
      console.log('[HlsTranscoder] VideoToolbox encoder detected - hardware decode disabled, using software decode + pooled frames')
    }
    ...
    if (usesVideoToolboxDecode) {
      try {
        ...
        videoDecoder.hwDeviceCtx = hwDevice
        videoDecoder.getFormat = (ctx, formats) => {
          if (!videoToolboxGetFormatLogged) {
            const formatNames = formats
              .map((fmt) => {
                try { return ffmpeg.constants.getPixelFormatName(fmt) } catch { return String(fmt) }
              })
              .join(', ')
            console.log('[HlsTranscoder] VideoToolbox decoder formats:', formatNames)
            videoToolboxGetFormatLogged = true
          }
          const hwFormat = formats.find((f) => f === ffmpeg.constants.pixelFormats.VIDEOTOOLBOX)
          return hwFormat || formats[0]
        }
        console.log('[HlsTranscoder] VideoToolbox decode enabled')
      } catch (hwErr) {
        console.warn('[HlsTranscoder] VideoToolbox decode setup failed, falling back to software:', hwErr?.message)
        hwDevice = null
      }
    } else if (usesVideoToolbox) {
      console.log('[HlsTranscoder] VideoToolbox decode disabled - using software decoder output')
    }
    ...
    if (!h264Selection.isHardware) {
      ...
      const hwOptions = [
        ...
        // Force periodic keyframes (MediaCodec option names vary by build)
        ['i_frame_interval', '2'], // Alternative to i-frame-interval (seconds)
        ['force_key_frames', 'expr:gte(t,n_forced*2)'], // Generic FFmpeg keyframe forcing
    ...
    try {
      if (videoEncoder.extraData && videoEncoder.extraData.length > 0) {
        ...
        if (extradata[0] === 0x01) {
          ...
          avccLengthSize = getAvccLengthSize(extradata)
    ...
    const videoToolboxPixelFormat = ffmpeg.constants.pixelFormats.VIDEOTOOLBOX
    ...
    if (!inputPixelFormat || inputPixelFormat === NONE || inputPixelFormat === 0 || inputPixelFormat < 0) {
      if (usesVideoToolboxDecode) {
        inputPixelFormat = videoToolboxPixelFormat
        console.log('[HlsTranscoder] Decoder format unknown (' + decoderPixelFormat + '), assuming VideoToolbox (' + inputPixelFormat + ')')
      } else {
    ...
    let scalerInputPixelFormat = inputPixelFormat
    let scalerInputWidth = targetWidth
    let scalerInputHeight = targetHeight

    const getPixelFormatName = (formatId) => {
      try {
        return ffmpeg.constants.getPixelFormatName(formatId)
      } catch {
        return String(formatId)
      }
    }
    const getPixelFormatNameLower = (formatId) => getPixelFormatName(formatId).toLowerCase()

    const selectVideoToolboxTransferFormat = (hwFrame) => {
      if (!usesVideoToolboxDecode) return null
      if (videoToolboxTransferFormat) return videoToolboxTransferFormat

      const hwFramesCtx = hwFrame?.hwFramesCtx
      if (!hwFramesCtx) {
        videoToolboxTransferFormat = encoderPixelFormat
        videoToolboxTransferFormatName = getPixelFormatName(encoderPixelFormat)
        videoToolboxTransferFormatSource = 'fallback:no-hw-frames-ctx'
        console.warn('[HlsTranscoder] VideoToolbox frame missing hwFramesCtx, using encoder format:', videoToolboxTransferFormatName)
        return videoToolboxTransferFormat
      }

      const NONE = ffmpeg.constants.pixelFormats.NONE
      const swFormat = hwFramesCtx.swFormat
      if (swFormat && swFormat !== NONE && swFormat > 0 && !videoToolboxTransferFailures.has(swFormat)) {
        videoToolboxTransferFormat = swFormat
        videoToolboxTransferFormatName = getPixelFormatName(swFormat)
        videoToolboxTransferFormatSource = 'hwFramesCtx.swFormat'
        console.log('[HlsTranscoder] VideoToolbox transfer SW format:', videoToolboxTransferFormatName, 'source:', videoToolboxTransferFormatSource)
        return videoToolboxTransferFormat
      }
      ...
      let validSwFormats = []
      try {
        constraints = hwFramesCtx.getConstraints()
        validSwFormats = Array.from(constraints.validSwFormats || [])
      } catch (err) {
        console.warn('[HlsTranscoder] Failed to read VideoToolbox constraints:', err?.message || err)
      } finally {
        if (constraints) {
          try { constraints.destroy() } catch {}
        }
      }

      videoToolboxValidFormats = validSwFormats

      const formatMatches = (fmt, needles) => {
        const name = getPixelFormatNameLower(fmt)
        return needles.some((needle) => name.includes(needle))
      }

      const prefer10Bit = videoBitDepth >= 10
      const has10BitFormat = validSwFormats.some((fmt) =>
        formatMatches(fmt, ['p010', 'p016', 'yuv420p10', 'yuv422p10', 'yuv444p10'])
      )

      const preferred10BitNames = ['p010', 'p016', 'yuv420p10', 'yuv422p10', 'yuv444p10']
      ...
      let selected = null
      if (prefer10Bit || has10BitFormat) {
        selected = validSwFormats.find((fmt) => formatMatches(fmt, preferred10BitNames) && !videoToolboxTransferFailures.has(fmt))
      }
      if (!selected) {
        selected = preferredFormats.find((fmt) => validSwFormats.includes(fmt) && !videoToolboxTransferFailures.has(fmt))
      }
      if (!selected && validSwFormats.length > 0) {
        selected = validSwFormats.find((fmt) => !videoToolboxTransferFailures.has(fmt)) || validSwFormats[0]
      }
      if (!selected) {
        selected = encoderPixelFormat
        videoToolboxTransferFormatSource = 'fallback:no-valid-formats'
      } else {
        videoToolboxTransferFormatSource = 'constraints'
      }

      videoToolboxTransferFormat = selected
      videoToolboxTransferFormatName = getPixelFormatName(selected)

      if (validSwFormats.length > 0) {
        const swFormatNames = validSwFormats.map((fmt) => getPixelFormatName(fmt)).join(', ')
        console.log('[HlsTranscoder] VideoToolbox valid SW formats:', swFormatNames)
      }
      console.log('[HlsTranscoder] VideoToolbox transfer SW format:', videoToolboxTransferFormatName, 'source:', videoToolboxTransferFormatSource)
    ...
    const ensureScalerForFormat = (frameFormat, frameWidth, frameHeight) => {
      const NONE = ffmpeg.constants.pixelFormats.NONE
      if (!frameFormat || frameFormat === NONE || frameFormat < 0) return
      if (frameFormat === videoToolboxPixelFormat && usesVideoToolboxDecode) return
      const sourceWidth = frameWidth || targetWidth
      const sourceHeight = frameHeight || targetHeight
      if (
        frameFormat === scalerInputPixelFormat &&
        sourceWidth === scalerInputWidth &&
        sourceHeight === scalerInputHeight
      ) {
        return
      }

      if (scaler) {
        try { scaler.destroy() } catch {}
        scaler = null
      }

      scalerInputPixelFormat = frameFormat
      scalerInputWidth = sourceWidth
      scalerInputHeight = sourceHeight
      if (frameFormat !== encoderPixelFormat || sourceWidth !== targetWidth || sourceHeight !== targetHeight) {
        console.log('[HlsTranscoder] Recreating scaler:', frameFormat, '->', encoderPixelFormat,
        ...
        console.log('[HlsTranscoder] Scaler not needed, formats match:', frameFormat)
      }
    }

    const pendingVideoFrames = useFreshEncoderFrames ? [] : null
    const reusableVideoFrames = useFreshEncoderFrames ? [] : null
    const retiredVideoFrames = useFreshEncoderFrames ? [] : null
    const maxPendingVideoFrames = useFreshEncoderFrames ? 96 : 12
    const maxReusableVideoFrames = useFreshEncoderFrames ? 64 : 0
    const prepareTransferFrame = (frame, format, width, height, shouldAlloc = false, assignDimensions = true) => {
      safeUnref(frame)
      frame.format = format
      if (assignDimensions) {
        frame.width = width
    ...
    const acquireVideoFrame = (format, width, height, shouldAlloc = false, assignDimensions = true) => {
      let frame = null
      if (useFreshEncoderFrames && reusableVideoFrames?.length) {
        frame = reusableVideoFrames.pop()
      } else {
        frame = new ffmpeg.Frame()
      }
      prepareTransferFrame(frame, format, width, height, shouldAlloc, assignDimensions)
      return frame
    }
    const allocateVideoFrame = (format, width, height) => {
      if (useFreshEncoderFrames) {
        return acquireVideoFrame(format, width, height, true, true)
      }
    ...
    const createTransferFrame = (format, width, height, shouldAlloc = false, assignDimensions = true) => {
      if (useFreshEncoderFrames) {
        return acquireVideoFrame(format, width, height, shouldAlloc, assignDimensions)
      }
      const frame = new ffmpeg.Frame()
      prepareTransferFrame(frame, format, width, height, shouldAlloc, assignDimensions)
      return frame
    }
    const recordVideoToolboxTransferFailure = (format) => {
      if (!format && format !== 0) return
      videoToolboxTransferFailures.add(format)
      videoToolboxTransferFormat = null
      videoToolboxTransferFormatName = null
      videoToolboxTransferFormatSource = null
    }
    const recycleVideoFrame = (frame) => {
      if (!useFreshEncoderFrames || !frame) return
      if (frame === videoFrame || frame === scaledFrame || frame === videoToolboxTransferFrame) return
      if (Object.prototype.hasOwnProperty.call(frame, '_handle') && !frame._handle) return
      safeUnref(frame)
      if (reusableVideoFrames.length < maxReusableVideoFrames) {
        reusableVideoFrames.push(frame)
      } else {
        safeDestroy(frame)
      }
    }
    const retireVideoFrame = (frame) => {
      if (!useFreshEncoderFrames || !frame) return
      if (frame === videoFrame || frame === scaledFrame || frame === videoToolboxTransferFrame) return
      if (Object.prototype.hasOwnProperty.call(frame, '_handle') && !frame._handle) return
      retiredVideoFrames.push(frame)
    }
    const trackPendingVideoFrame = (frame) => {
      if (!useFreshEncoderFrames || !frame) return
      if (frame === videoFrame || frame === scaledFrame || frame === videoToolboxTransferFrame) return
      pendingVideoFrames.push(frame)
      if (pendingVideoFrames.length > maxPendingVideoFrames) {
        const dropped = pendingVideoFrames.shift()
        console.warn('[HlsTranscoder] VideoToolbox encoder frame queue overflow; retiring oldest frame')
        retireVideoFrame(dropped)
      }
    }
    const releasePendingVideoFrame = () => {
      if (!useFreshEncoderFrames || !pendingVideoFrames || pendingVideoFrames.length === 0) return
      const frame = pendingVideoFrames.shift()
      recycleVideoFrame(frame)
    }
    releaseAllPendingVideoFrames = (finalize = false) => {
      if (!useFreshEncoderFrames || !pendingVideoFrames) return
      while (pendingVideoFrames.length) {
        const frame = pendingVideoFrames.shift()
        if (finalize) {
          safeDestroy(frame)
        } else {
      ...
      if (finalize) {
        while (reusableVideoFrames.length) safeDestroy(reusableVideoFrames.pop())
        while (retiredVideoFrames.length) safeDestroy(retiredVideoFrames.pop())
      }
    }

    // Always create scaler if formats differ (except VideoToolbox decode, which transfers first)
    if (usesVideoToolboxDecode && inputPixelFormat === videoToolboxPixelFormat) {
      console.log('[HlsTranscoder] VideoToolbox decoder output detected - delaying scaler until transfer')
    } else if (inputPixelFormat !== encoderPixelFormat) {
      console.log('[HlsTranscoder] Creating scaler:', inputPixelFormat, '->', encoderPixelFormat, '(' + targetWidth + 'x' + targetHeight + ')')
      // Scaler args: srcPixelFormat, srcWidth, srcHeight, dstPixelFormat, dstWidth, dstHeight
      scaler = new ffmpeg.Scaler(
        inputPixelFormat,
        targetWidth,
    ...
    convertedPacket = new ffmpeg.Packet()
    ...
    if (hasAudio) {
      ...
      // Use a larger fixed buffer to avoid resampler overruns on high-delay inputs.
      // 16384 samples ~= 0.34s @ 48kHz, still small in memory terms.
      resampledFrame.nbSamples = 16384
      resampledFrame.alloc()
      console.log('[HlsTranscoder] resampledFrame allocated with 16384 samples buffer')
    ...
    while (inputFormat.readFrame(packet)) {
      ...
      if (packet.streamIndex === videoStream.index) {
        ...
        try {
          if (videoDecoder.sendPacket(packet)) {
            ...
            while (videoDecoder.receiveFrame(videoFrame)) {
              ...
              // Ensure scaler matches actual decoded frame format (HEVC often differs from metadata)
              ensureScalerForFormat(videoFrame.format, videoFrame.width, videoFrame.height)

              let frameToEncode = videoFrame

              if (videoFrame.format === videoToolboxPixelFormat && usesVideoToolboxDecode) {
                if (packetCount <= 5) console.log('[HlsTranscoder] Transferring VideoToolbox frame to software buffer...')
                let transferFrame = null
                try {
                  const transferFormat = selectVideoToolboxTransferFormat(videoFrame)
                  const transferWidth = videoFrame.width || targetWidth
                  const transferHeight = videoFrame.height || targetHeight
                  const shouldAllocTransfer = !videoToolboxUseHwMap
                  const assignDimensions = true
                  if (useFreshEncoderFrames) {
                    transferFrame = createTransferFrame(transferFormat, transferWidth, transferHeight, shouldAllocTransfer, assignDimensions)
                  } else {
                    if (!videoToolboxTransferFrame) {
                      videoToolboxTransferFrame = new ffmpeg.Frame()
                    }
                    transferFrame = videoToolboxTransferFrame
                    prepareTransferFrame(transferFrame, transferFormat, transferWidth, transferHeight, shouldAllocTransfer, assignDimensions)
                  }

                  if (videoToolboxUseHwMap) {
                    videoFrame.hwMap(transferFrame, ffmpeg.constants.hwFrameMapFlags.READ)
                  } else {
                    videoFrame.transferData(transferFrame)
                  }
                  const shouldReleaseTransferFrame = useFreshEncoderFrames ? transferFrame : null

                  const originalPts = videoFrame.pts || 0
                  const rescaledPts = Math.round((originalPts * rescaleNum) / rescaleDen)

                  const actualTransferFormat = transferFrame.format || transferFormat
                  const actualTransferWidth = transferFrame.width || transferWidth
                  const actualTransferHeight = transferFrame.height || transferHeight

                  const needsScaling =
                    actualTransferFormat !== encoderPixelFormat ||
                    actualTransferWidth !== targetWidth ||
                    actualTransferHeight !== targetHeight

                  if (needsScaling) {
                    ensureScalerForFormat(actualTransferFormat, actualTransferWidth, actualTransferHeight)
                    if (!scaler) {
                      console.warn('[HlsTranscoder] VideoToolbox scaler missing, encoding transfer frame directly')
                      transferFrame.pts = rescaledPts
                      transferFrame.timeBase = encoderTB
                      frameToEncode = transferFrame
                    } else {
                      if (packetCount <= 5) console.log('[HlsTranscoder] Scaling transferred frame to', getPixelFormatName(encoderPixelFormat))
                      const scaledTarget = useFreshEncoderFrames
                        ? allocateVideoFrame(encoderPixelFormat, targetWidth, targetHeight)
                        : scaledFrame
                      scaler.scale(transferFrame, scaledTarget)
                      scaledTarget.pts = rescaledPts
                      scaledTarget.timeBase = encoderTB
                      frameToEncode = scaledTarget
                    }
                  } else {
                    transferFrame.pts = rescaledPts
                    transferFrame.timeBase = encoderTB
                  ...
                  if (shouldReleaseTransferFrame && frameToEncode !== shouldReleaseTransferFrame) {
                    recycleVideoFrame(shouldReleaseTransferFrame)
                  }

                  if (packetCount <= 5) {
                    const transferName = videoToolboxTransferFormatName || getPixelFormatName(actualTransferFormat)
                    console.log('[HlsTranscoder] Frame transferred, pts:', originalPts, '->', rescaledPts, 'format:', transferName)
                  }
                } catch (transferErr) {
                  const transferMethod = videoToolboxUseHwMap ? 'hwMap' : 'transferData'
                  console.error('[HlsTranscoder] VideoToolbox', transferMethod, 'error at frame', videoFrameCount, ':', transferErr?.message || transferErr)
                  if (!videoToolboxUseHwMap) {
                    videoToolboxTransferErrorCount += 1
                    if (videoToolboxTransferErrorCount >= 3) {
                      videoToolboxUseHwMap = true
                      console.warn('[HlsTranscoder] VideoToolbox transferData failing; switching to hwMap for this session')
                    }
                  }
                  recordVideoToolboxTransferFailure(videoToolboxTransferFormat)
                  if (useFreshEncoderFrames && transferFrame) {
                    recycleVideoFrame(transferFrame)
                  }
                  videoFrame.unref()
                  continue
                }
                ...
                let scaledTarget = null
                try {
                  scaledTarget = useFreshEncoderFrames
                    ? allocateVideoFrame(encoderPixelFormat, targetWidth, targetHeight)
                    : scaledFrame
                  scaler.scale(videoFrame, scaledTarget)
                  ...
                  if (useFreshEncoderFrames && scaledTarget && scaledTarget !== scaledFrame) {
                    recycleVideoFrame(scaledTarget)
                  }
                  videoFrame.unref()
            ...
            try {
              ...
              const frameAccepted = videoEncoder.sendFrame(frameToEncode)
              if (frameAccepted) {
                trackPendingVideoFrame(frameToEncode)
                if (packetCount <= 5) console.log('[HlsTranscoder] Frame sent, receiving packets...')
                let releasedFrameForSend = false
                while (videoEncoder.receivePacket(outputPacket)) {
                  ...
                  if (!outputPacket.data || packetSize === 0) {
                    ...
                    outputPacket.unref()
                    if (useFreshEncoderFrames && !releasedFrameForSend) {
                      releasePendingVideoFrame()
                      releasedFrameForSend = true
                    }
                    continue
                  }
                  let packetData = outputPacket.data
                  let packetDataChanged = false
                  let packetIsAnnexB = looksLikeAnnexB(packetData)

                  if (!packetIsAnnexB && avccLengthSize) {
                    const converted = convertAvccPacketToAnnexB(packetData, avccLengthSize)
                    if (converted) {
                      packetData = converted
                      packetDataChanged = true
                      packetIsAnnexB = true
                    }
                  }

                  let nalFlags = scanH264NalFlags(packetData)

                  if (!packetIsAnnexB && nalFlags.lengthSize) {
                    const converted = convertAvccPacketToAnnexB(packetData, nalFlags.lengthSize)
                    if (converted) {
                      packetData = converted
                      packetDataChanged = true
                      packetIsAnnexB = true
                      nalFlags = scanH264NalFlags(packetData)
                    }
                  }

                  let isKeyframe = outputPacket.isKeyframe
                  if (!isKeyframe && nalFlags.hasIDR) {
                    isKeyframe = true
                    if (totalEncoderPackets <= 5 || totalEncoderPackets % 500 === 0) {
                      console.log('[HlsTranscoder] Keyframe flag missing - IDR detected, treating as keyframe')
                    }
                  }
                  ...
                  const rawEncoderPts = Number.isFinite(outputPacket.pts) ? outputPacket.pts : null
                  let encoderPtsMs = rawEncoderPts

                  // Convert to seconds for segment timing calculation
                  // Encoder timebase is 1/1000, so pts in seconds = outputPacket.pts / 1000
                  if (encoderPtsMs === null || encoderPtsMs <= 0) {
                    encoderPtsMs = currentFramePtsMs
                  }
                  if (lastVideoPtsMs !== null && Number.isFinite(lastVideoPtsMs) && encoderPtsMs <= lastVideoPtsMs) {
                    encoderPtsMs = lastVideoPtsMs + 1
                  }
                  const usedFallbackPts = encoderPtsMs !== rawEncoderPts
                  ...
                  if (!isKeyframe && !noKeyframeWarned && segmentDuration >= TARGET_SEGMENT_DURATION * 2.5) {
                    console.warn('[HlsTranscoder] No keyframe after', segmentDuration.toFixed(2),
                      's (segment 0 still open) - encoder keyframes may be too sparse')
                    noKeyframeWarned = true
                  }
                  ...
                  if (isKeyframe) {
                    ...
                    try {
                      const { hasSPS, hasPPS, hasIDR } = nalFlags

                      // Log for first few keyframes or periodically
                      const shouldLogKeyframe = totalEncoderPackets <= 5 || segmentIndex <= 3 || segmentIndex % 10 === 0
                      if (shouldLogKeyframe) {
                      ...
                      if (packetIsAnnexB && !hasSPS && !hasPPS && spsPpsNalus && spsPpsNalus.length > 0) {
                        const injected = Buffer.concat([spsPpsNalus, packetData])
                        packetData = injected
                        packetDataChanged = true
                        packetIsAnnexB = true
                        if (shouldLogKeyframe || totalEncoderPackets % 500 === 0) {
                          console.log('[HlsTranscoder] Injected SPS/PPS into keyframe #' + totalEncoderPackets)
                        }
                      }
                    } catch (nalCheckErr) {
                  ...
                  // Use encoder PTS (or fallback) for muxing timestamps
                  // Set stream index and DTS (= PTS since B-frames disabled)
                  packetToWrite.streamIndex = continuousMuxer.videoStream.index
                  packetToWrite.isKeyframe = isKeyframe
                  packetToWrite.pts = encoderPtsMs
                  // Set DTS = PTS (no B-frames means no reordering)
                  packetToWrite.dts = packetToWrite.pts
                  packetToWrite.timeBase = videoEncoder.timeBase  // 1/1000
                  ...
                  try {
                    continuousMuxer.format.writeFrame(packetToWrite)
                  } catch (writeErr) {
              ...
              } else if (useFreshEncoderFrames && frameToEncode && frameToEncode !== videoFrame && frameToEncode !== scaledFrame && frameToEncode !== videoToolboxTransferFrame) {
                recycleVideoFrame(frameToEncode)
              }
            } catch (encodeErr) {
        ...
        const packetPtsRaw = Number.isFinite(packet.pts) ? packet.pts : (Number.isFinite(packet.dts) ? packet.dts : null)
        const packetTimeBase = audioStream.timeBase || { numerator: 1, denominator: 48000 }
        const packetPtsMs = (packetPtsRaw !== null && packetPtsRaw >= 0)
          ? (packetPtsRaw * packetTimeBase.numerator * 1000) / packetTimeBase.denominator
          : null

        // Set base PTS from FIRST audio packet only
        if (audioBasePtsMs === null && packetPtsMs !== null) {
          audioBasePtsMs = packetPtsMs
          if (audioPtsOffset === null && audioEncoder?.timeBase) {
            const audioEncTB = audioEncoder.timeBase
            const baseMs = Math.max(0, audioBasePtsMs)
            audioPtsOffset = Math.round((baseMs / 1000) * (audioEncTB.denominator / audioEncTB.numerator))
            if (!Number.isFinite(audioPtsOffset)) audioPtsOffset = 0
          }
        ...
        if (totalAudioPackets <= 5) {
          console.log('[HlsTranscoder] Audio packet', totalAudioPackets,
            'pts:', packetPtsRaw ?? 'n/a',
            'ptsMs:', packetPtsMs !== null ? packetPtsMs.toFixed(1) : 'n/a',
            'basePtsMs:', audioBasePtsMs !== null ? audioBasePtsMs.toFixed(1) : 'n/a')
        }
          ...
          if (audioDecoder.sendPacket(packet)) {
            while (audioDecoder.receiveFrame(audioFrame)) {
              ...
              if (audioFrameCount <= 3) {
                ...
                let estimatedOutSamples = Math.ceil((resamplerDelay + inputSamples) * outRate / inRate)
                if (!Number.isFinite(estimatedOutSamples) || estimatedOutSamples <= 0) {
                  estimatedOutSamples = resampledFrame.nbSamples
                }
                if (estimatedOutSamples > resampledFrame.nbSamples) {
                  const newCapacity = Math.max(estimatedOutSamples, 16384)
                  resampledFrame.unref()
                  resampledFrame.format = audioEncoder.sampleFormat
                  resampledFrame.channelLayout = ffmpeg.constants.channelLayouts.STEREO
                  resampledFrame.sampleRate = audioEncoder.sampleRate
                  resampledFrame.nbSamples = newCapacity
                  resampledFrame.alloc()
                  console.log('[HlsTranscoder] ResampledFrame reallocated:', newCapacity, 'samples (estimated)')
                }

                // Save allocated buffer size before convert modifies nbSamples
                const resampledBufferSize = resampledFrame.nbSamples

                let samplesConverted = resampler.convert(audioFrame, resampledFrame)
                if (audioFrameCount <= 2 || audioFrameCount % 2000 === 0) {
                  console.log('[HlsTranscoder] Audio convert: inputSamples=', audioFrame.nbSamples,
                    'bufferSize=', resampledBufferSize, 'converted=', samplesConverted)
                }

                if (!Number.isFinite(samplesConverted) || samplesConverted <= 0) {
                  if (samplesConverted < 0) {
                    console.warn('[HlsTranscoder] Audio convert failed:', samplesConverted, 'samples')
                  }
                  resampledFrame.nbSamples = resampledBufferSize
                  continue
                }

                if (samplesConverted > resampledBufferSize) {
                  console.warn('[HlsTranscoder] Audio convert overflow:', samplesConverted,
                    '>', resampledBufferSize, '- clamping')
                  samplesConverted = resampledBufferSize
                }
              ...
              while (audioFifo.size >= aacFrameSize && framesEncodedThisIteration < maxFramesPerIteration) {
                ...
                if (audioEncoder.sendFrame(encoderFrame)) {
                  ...
                  while (audioEncoder.receivePacket(outputPacket)) {
                    ...
                    const rawPts = outputPacket.pts
                    if (!Number.isFinite(rawPts) || rawPts < 0) {
                      if (totalAudioEncoderPackets <= 2) {
                        console.log('[HlsTranscoder] Audio priming packet skipped, pts:', rawPts)
                      }
                      outputPacket.unref()
                      continue
                    }

                    const audioEncTB = audioEncoder.timeBase || { numerator: 1, denominator: encoderSampleRate }
                    if (audioPtsOffset === null) {
                      const baseMs = (Number.isFinite(audioBasePtsMs) && audioBasePtsMs >= 0)
                        ? audioBasePtsMs
                        : (Number.isFinite(firstVideoPtsMs) && firstVideoPtsMs >= 0 ? firstVideoPtsMs : 0)
                      audioPtsOffset = Math.round((baseMs / 1000) * (audioEncTB.denominator / audioEncTB.numerator))
                      if (!Number.isFinite(audioPtsOffset)) audioPtsOffset = 0
                    }
                    const adjustedPts = rawPts + (audioPtsOffset || 0)
                    outputPacket.streamIndex = continuousMuxer.audioStream.index
                    const mpegtsTimeBase = { numerator: 1, denominator: 90000 }
                    outputPacket.pts = adjustedPts
                    outputPacket.dts = adjustedPts
                    ...
                    if (totalAudioEncoderPackets <= 5 || totalAudioEncoderPackets % 200 === 0) {
                      console.log('[HlsTranscoder] Audio encoder packet', totalAudioEncoderPackets,
                        'rawPts:', rawPts, 'adjPts:', adjustedPts, 'pts90k:', outputPacket.pts,
      ...
      // Log progress every 500 packets for debugging
    ...
    if (session.cancelled) {
      console.log('[HlsTranscoder] Transcode cancelled, skipping finalization')
      return
    }
    ...
    // Frames first
    if (releaseAllPendingVideoFrames) {
      releaseAllPendingVideoFrames(true)
    }
    ...
    if (videoToolboxTransferFrame) { try { videoToolboxTransferFrame.destroy() } catch {} videoToolboxTransferFrame = null }
    if (hwDevice) { try { hwDevice.destroy() } catch {} hwDevice = null }
    if (videoFrame) { try { videoFrame.destroy() } catch {} videoFrame = null }
    if (convertedPacket) { try { convertedPacket.destroy() } catch {} convertedPacket = null }
    ...
    if (continuousMuxer?.format?._handle) {
      safeDestroy(continuousMuxer.format)
      continuousMuxer = null
    }
...
export async function startHlsTranscode(sourceUrl, options = {}) {
  ...
  for (const [id, session] of sessions) {
    if (isSessionMatch(session, sourceKey, sourceUrl) && session.status !== 'error' && !session.cancelled) {
      if (sourceKey && session.sourceUrl !== sourceUrl) {
        session.sourceUrl = sourceUrl
      }
      console.log('[HlsTranscoder] Reusing existing session for source:', id, 'status:', session.status)
  ...
  if (sessions.size > 0) {
    ...
    for (const [id, session] of sessions) {
      console.log('[HlsTranscoder] Cancelling old session:', id)
      stopHlsTranscode(id, { reason: 'replaced' })
    }
  }
  ...
  const session = {
    ...
    forceSoftwareDecode,
    forceHypercoreStream,
    hypercoreStreamReader: null,
    createdAt: Date.now(),
    cancelled: false,
    cancelReason: null,
    inputCleaned: false,
    segmentsDestroyed: false,
  }
  ...
  ;(async () => {
    ...
    try {
      if (session.cancelled) {
        ...
        session.error = session.cancelReason || 'cancelled'
        return
      }
      // ============================================
      ...
      if (!inputIO) {
        ...
        const waitForComplete = isVideoComplete && !forceProgressive
        const streamReader = new TempFileReader(sourceUrl, fileSize, {
          waitForComplete
        })
        session.streamReader = streamReader
        console.log('[HlsTranscoder] TempFileReader created, waitForComplete:', waitForComplete, 'forceProgressive:', forceProgressive)
      ...
      const detection = await detectTranscodeNeeded(sourceUrl, title)
      ...
      if (session.cancelled) {
        session.status = 'cancelled'
        session.error = session.cancelReason || 'cancelled'
        console.log('[HlsTranscoder] Session cancelled:', sessionId, session.error)
      } else {
      ...
      const shouldDestroySegments = session.status === 'error' || session.status === 'cancelled'
      const shouldRemove = shouldDestroySegments
      cleanupSession(session, {
        destroySegments: shouldDestroySegments,
        remove: shouldRemove,
      })
    }
  })()
 ...
 */
export function stopHlsTranscode(sessionId, options = {}) {
  const session = sessions.get(sessionId)
  if (!session) {
  ...
  const destroySegments = typeof options === 'object' && typeof options.destroySegments === 'boolean'
    ? options.destroySegments
    : true

  if (session.status === 'complete' || session.status === 'error') {
    cleanupSession(session, { destroySegments, remove: true })
    return { success: true }
  }

  markSessionCancelled(session, reason)
  return { success: true }
}
