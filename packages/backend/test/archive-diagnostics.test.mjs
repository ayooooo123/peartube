...
test('archive policy reports capacity gauges, exhaustion, and rejected reservations', async (t) => {
  ...
  t.is(policy.reserve({ pledgeId: 'p1', bytes: 8, expiresAt: 50 }).accepted, true)
  t.is(policy.reserve({ pledgeId: 'p2', bytes: 3, expiresAt: 50 }).reason, 'capacity-exceeded')
  t.is(policy.reserve({ pledgeId: '', bytes: 1, expiresAt: 50 }).reason, 'invalid-reservation')
...
test('archive manager accounts for bounded offload rejection reasons', async (t) => {
  ...
  t.is(manager.confirmSourceOffload({ assessmentId: 'missing' }).reason, 'assessment-not-found')
  const assessment = manager.createOffloadAssessment({
    publicationId: 'a'.repeat(64),
    evidence: { durable: false },
    expiresAt: 50,
  })
  t.is(manager.confirmSourceOffload({
    publicationId: 'a'.repeat(64),
