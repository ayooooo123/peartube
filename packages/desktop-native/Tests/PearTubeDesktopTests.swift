...
final class PearTubeDesktopTests: XCTestCase {
  ...
  func testMpvPlaybackSignalRequiresDecodedStateOrFrame() {
    XCTAssertFalse(
      HostBridgeService.mpvSessionHasPlaybackSignal(
  ...
  func testNativePlaybackDefaultsToAVPlayerUnlessMpvIsExplicitlyEnabled() {
    let unknownVideo = NativeVideo(
      id: "channel-a:video-unknown",
      backendVideoID: "video-unknown",
      channelKey: "channel-a",
      title: "Unknown",
      channelName: "Channel A",
      durationText: "1:23",
    ...
    let mp4Video = NativeVideo(
      id: "channel-a:video-mp4",
      backendVideoID: "video-mp4",
      channelKey: "channel-a",
      title: "MP4",
      channelName: "Channel A",
      durationText: "1:23",
      summary: "Known format",
      tags: ["test"],
      accentHex: "#FF7A59",
      sections: [.home],
      path: "/videos/video-mp4.mp4",
      mimeType: "video/mp4"
    )
    let publicFeedMp4Video = NativeVideo(
      id: "channel-public:video-mp4",
      backendVideoID: "video-mp4",
      channelKey: "channel-public",
      publicBeeKey: "public-bee-key",
      title: "Public Feed MP4",
      channelName: "Public Channel",
      durationText: "1:23",
      summary: "Known format from the public feed",
      tags: ["test"],
      accentHex: "#FF7A59",
      sections: [.home],
      path: "/videos/video-mp4.mp4",
    ...
    XCTAssertFalse(HostBridgeService.shouldUseNativeMpvPlayback(for: unknownVideo, environment: [:]))
    XCTAssertFalse(HostBridgeService.shouldUseNativeMpvPlayback(for: mp4Video, environment: [:]))
    XCTAssertFalse(HostBridgeService.shouldUseNativeMpvPlayback(for: publicFeedMp4Video, environment: [:]))
    XCTAssertFalse(HostBridgeService.shouldUseNativeMpvPlayback(for: unknownVideo, environment: ["PEARTUBE_NATIVE_ENABLE_MPV": "0"]))
    XCTAssertFalse(HostBridgeService.shouldUseNativeMpvPlayback(for: mp4Video, environment: ["PEARTUBE_NATIVE_ENABLE_MPV": "false"]))
    XCTAssertFalse(HostBridgeService.shouldUseNativeMpvPlayback(for: publicFeedMp4Video, environment: ["PEARTUBE_NATIVE_ENABLE_MPV": "0"]))
    XCTAssertTrue(HostBridgeService.shouldUseNativeMpvPlayback(for: unknownVideo, environment: ["PEARTUBE_NATIVE_ENABLE_MPV": "1"]))
