...
├── spec/             # HRPC schema definitions + JS codegen
...
2. **HRPC RPC**: Type-safe RPC via `@peartube/spec`. Schema defined in `packages/spec/schema.cjs`, generates JS (`spec/hrpc/`, `spec/schema/`) from a single source of truth.
...
| `packages/spec/schema.cjs` | HRPC schema definition (single source of truth for JS) |
