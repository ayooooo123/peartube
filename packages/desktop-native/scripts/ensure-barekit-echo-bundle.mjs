...
const entryFile = path.join(packageRoot, 'Bridge', 'barekit-echo-worklet.cjs')
const bundleFile = path.join(packageRoot, 'Resources', 'Generated', 'barekit-echo.bundle')
...
if (forced || missingBundle || staleBundle) {
  ...
  console.log(`[bundle:barekit-echo:ensure] Rebuilding (${reason})`)
  runBarePack()
} else {
  console.log('[bundle:barekit-echo:ensure] BareKit echo bundle is up to date')
}
