...
function validateBlobAndRange(blob, byteRange) {
  ...
  return { blockOffset, blockLength, byteLength, rangeStart, rangeEnd }
}

function computeEffectiveRangeStart(rangeStart, options) {
  const snapOffsets = Array.isArray(options.snapOffsets) && options.snapOffsets.length > 0
    ? options.snapOffsets
    : null
  if (!snapOffsets || rangeStart <= 0) return rangeStart
...
export function getPrioritizedBlobDownloadRange(blob, byteRange, options = {}) {
  const validated = validateBlobAndRange(blob, byteRange)
  if (!validated) return null

  const { blockOffset, blockLength, byteLength, rangeStart, rangeEnd } = validated
  const effectiveRangeStart = computeEffectiveRangeStart(rangeStart, options)
...
function canReuseExistingPriorityRange(existing, downloadRange, transient) {
  if (transient || !existing?.range) return false
  return (
...
async function acquirePriorityCore(blobServer, request, existing, registryKey, transient) {
...
function registerPriorityEntry(registryKey, entry, existing) {
...
function attachPriorityCleanup(entry, core, range, transient, options) {
  const timeoutMs = Math.max(
    1000,
    Number(options.timeoutMs ?? DEFAULT_BLOB_RANGE_PRIORITY_TIMEOUT_MS) || DEFAULT_BLOB_RANGE_PRIORITY_TIMEOUT_MS
  )
...
export async function prioritizeBlobServerRangeRequest(blobServer, req, options = {}) {
  ...
  if (canReuseExistingPriorityRange(existing, downloadRange, transient)) {
    return downloadRange
  }

  const core = await acquirePriorityCore(blobServer, request, existing, registryKey, transient)
  if (!core) return null
  ...
  if (!transient) {
    registerPriorityEntry(registryKey, entry, existing)
  }

  attachPriorityCleanup(entry, core, range, transient, options)
