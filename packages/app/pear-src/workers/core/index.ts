...
if (originalPipeOn) {
  ipcPipe.on = (event: any, listener: any) => {
    if (event === 'data' && typeof listener === 'function') {
      return originalPipeOn(event, (chunk: any) => {
        if (chunk === undefined || chunk === null) { if (!sawInvalidPipeChunk) { sawInvalidPipeChunk = true; console.warn('[Worker] Dropping empty IPC pipe chunk') } return }
        listener(chunk)
      })
    }
