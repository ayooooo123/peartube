...
test('authorized rendition opens the canonical static core and attaches it directly to matching peers', async (t) => {
  ...
  t.alike(downloads, [], 'retaining a rendition does not eagerly download it')
  const connection = new EventEmitter()
  swarm.emit('connection', connection, { client: true })
  t.is(unrestrictedReplications, 1, 'the retained Hypercore attaches directly to the peer mux')
  t.alike(opened, [b4a.toString(allowedCore, 'hex')], 'no other core is opened')
  ...
  t.alike(destroyedDownloads, [], 'no eager download exists to destroy')
  await runtime.close()
})
