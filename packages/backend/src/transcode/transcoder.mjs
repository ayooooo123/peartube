...
const H264_PROFILE_NAME_MAP = new Map([
  [66, 'baseline'],
  [77, 'main'],
  [88, 'extended'],
  [100, 'high'],
...
async function getHttpContentLength(url) {
  ...
  if (parsedUrl.protocol === 'https:') {
    const mod = await import('bare-https')
 ...
 */
async function httpRangeReadSync(url, start, length) {
  let parsedUrl
  try {
    parsedUrl = new URL(url)
  } catch (e) {
 ...
 * @param {string} title - Video title (unused, kept for compatibility)
 */
export async function probeMedia(url, title = '', options = {}) {
  const result = {
    videoCodec: null,
    audioCodec: null,
    audioChannels: 0,
  ...
  if (!ffmpeg) {
    ...
    if (!loaded) {
      throw new Error(ffmpegLoadError || 'bare-ffmpeg not available for probe')
    }
  }

  const native = await probeWithBareFFmpeg(url)
  Object.assign(result, native)
  if (!result.videoCodec && !result.audioCodec) {
    throw new Error('bare-ffmpeg probe returned no codec information')
  }

  checkTranscodeNeeded(result)
  return result
}
