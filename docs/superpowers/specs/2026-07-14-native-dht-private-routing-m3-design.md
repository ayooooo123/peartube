...
4. After guard pinning, the client invokes `RELAY_DISCOVER_V1` over the authenticated guard link. The guard performs ordinary random-target DHT walks using its public DHT participation and returns bounded candidate-signed advertisements. These targets are random discovery probes, not application topics.
5. The client selects a `ROLE.SAFETY` middle and sends a nested link offer through the guard. The guard contacts the selected address; the middle proves advertisement possession through the partial route. The client never probes it directly.
6. Once the client→guard→middle control route exists, the client invokes the same bounded discovery service at the middle to obtain `ROLE.PRIVATE` exit candidates. Exit validation and link extension run through the partial route.
...
When all configured bootstrap nodes are legacy nodes, the client may perform one strictly bounded direct discovery walk: one legacy bootstrap query followed by at most three direct capability probes to nodes returned by that query, one at a time. Every contacted address is recorded in the privacy-readiness report because it saw the client IP. Discovery stops immediately when a guard is pinned. If no guard is found within the bound, private readiness fails closed. Applications may disable this legacy cold-start walk and require a compatible configured bootstrap.
...
The exit is both the final adjacent route participant and the owner of the end-to-end route payload codec. It is not modeled as an M2 private entry followed by an imaginary destination. M3 therefore extends activation with a `DHT_EXIT_V1` terminal confirmation that binds the exit service policy, branch class, branch generation, complete short-route transcript, and the same M2 key/cell parameters.
...
For the first link, the fresh client circuit identity is the initiator. For later links, the already-adjacent relay is the initiator after receiving a nested, end-to-end authenticated extension request from the client. The client accepts the extension only after receiving an `EXTENDED_V1` confirmation binding the selected advertisement and responder-signed acceptance. This authorizes only one adjacent link and reveals no complete path. A relay advertisement is an invitation to request bounded service, not a pre-issued topology grant.
...
The current guard, both branch middles, and both exits are excluded from DHT destination and private-storage handles for that client. A multi-capability route relay therefore cannot also receive that client's topic merely because it appears in a DHT response.
...
- while a branch has active work, heartbeat interval defaults to 25 seconds with bounded jitter;
- three missed heartbeat rounds mark a link failed;
- an otherwise idle branch is destroyed after 60 seconds without application or maintenance work;
- after idle destruction, the client emits zero route cells until new work arrives;
- no more than six heartbeat/control cells per branch per minute are permitted while a ready branch is otherwise idle;
...
On suspension the client cancels outstanding operations, destroys both branches, invalidates all destination handles and DHT/storage tokens, erases link and route secrets, and closes temporary IO. On resume it may reuse the pinned guard identity, but it must revalidate a current advertisement and active challenge and create fresh bilateral link acceptances, ephemeral keys, circuit IDs, generations, counters, handles, and tokens.

A network-interface change performs the same invalidation immediately. Mutating operations interrupted after prepare restart from prepare; signed storage receipts already obtained remain historical evidence but cannot be combined with a new transaction unless the new lookup confirms the same committed record and the normal quorum rules are satisfied.
...
- arbitrary exit destinations, address substitution, stale handles, cross-exit handles, relay-as-destination exclusion, and handle invalidation;
...
- compatible-storage overlay convergence, partial quorum, prepare/commit rotation, replayed receipts, cross-exit tokens, and insufficient density;
- quotas, response amplification, fairness, backpressure, and `BUSY` behavior;
- mobile liveness cell/wakeup budgets and zero traffic after idle destruction;
