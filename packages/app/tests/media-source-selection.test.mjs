...
import { selectMediaSource, normalizeMediaSource, scoreMediaSource, switchMediaSource } from '../lib/media-source-selection.js'
...
test('bounded source selection keeps playback bound to an authorized publication and rendition', () => {
  const sources = [
...
test('source switching preserves entity navigation identity separately from playback identity', () => {
  const next = switchMediaSource(
    { entityId: 'work:alpha', playbackRef: { publicationId: 'pub-a', renditionId: 'rend-a' } },
    { publicationId: 'pub-b', renditionId: 'rend-b', availabilityState: 'available', rejectionReasonCodes: [] },
  )
  assert.equal(next.entityId, 'work:alpha')
  assert.deepEqual(next.playbackRef, { publicationId: 'pub-b', renditionId: 'rend-b' })
})

test('bounded source selection fails closed when authorization diagnostics are missing', () => {
